...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (N_ncdm > 0){
    ...
    if (flag4 == _TRUE_){
      pba->f_yp = param4;
      if ((pba->f_yp != 0.0) && (pba->m_ncdm_in_eV[0] != 0.0)){     //If there is some decay into photons (f!=0), and we are considering a massive neutrino, then correct the T_ncdm
        N_ur2 = 3.044 - pow(11./4.,4./3.)*pow(0.71611,4.);   // Contribution to Neff at BBN from massless neutrinos. i.e. 3.044 - Neff_ncdm
 ...
 class_call(parser_read_double(pfc,"maxyr_yp",&param4,&flag4,errmsg),                            
  ...
  if ((flag4 == _TRUE_) && (pba->f_yp != 0.0)){ 
    ...
    pba->postNeff_yp = (1.0/gap_g)*(3.044+2.0308*(gap_ur-1.0));
